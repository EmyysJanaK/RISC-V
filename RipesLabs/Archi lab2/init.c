...
pintos_init (void)
{
  ...
  if (*argv != NULL) {
    ...
    run_actions (argv);
      ...
      time_t start = rtc_get_time(); // Get the current time
      while (1)
      {
        // TODO: no command line passed to kernel. Run interactively 
        printf("CS2043>>");// Print the prompt
        char* buffer = (char*) malloc(6*sizeof(char));// Allocate memory for the input buffer
        int i=0;
        char test;
        // Read characters until Enter key is pressed
        while (1)
        {
          test = (char)input_getc();
          if (test != '\r')
          {
            buffer[i] = test;
            putchar(test);
        ...
        buffer[i] = '\0';
        // Check user input and take corresponding actions
        ...
        {
          printf ("\nAmount of ram available for the OS  %'"PRIu32" kB RAM...\n",
          init_ram_pages * PGSIZE / 1024);
        }
